...
const Hero: React.FC<HeroProps> = ({ scrollToSection }) => {
  ...
  // Sync state with context, unless we've fallen back
  useEffect(() => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
        ...
        try {
            // Use original image (no compression) as requested
            const base64 = await compressImage(file, { keepOriginal: true });
  ...
  const handleError = () => {
      // If the local/configured image fails, fall back to a high-quality Unsplash image
      // This prevents the background from being blank/black if the file path is wrong
      console.warn("Hero image failed to load, switching to fallback.");
      setCurrentImgSrc('https://images.unsplash.com/photo-1599586120429-48285b6a8a81?q=80&w=1920&auto=format&fit=crop');
  };
