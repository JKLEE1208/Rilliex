...
import { Trophy, Medal, MapPin, Users, Award, Crown, Globe, Camera, Mail } from 'lucide-react';
...
const Achievements: React.FC = () => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
        try {
            // Keep original profile image quality (no compression)
            const base64 = await compressImage(e.target.files[0], { keepOriginal: true });
                    ...
                    <div className="p-8 flex-1 flex flex-col gap-6">
                        ...
                        <div className="bg-gradient-to-r from-yellow-500/10 to-transparent p-6 rounded-xl border border-yellow-500/20 relative overflow-hidden text-center md:text-left">
                            ...
                            <div className="flex items-center gap-3 mb-3 relative z-10 justify-center md:justify-start">
                                ...
                                <span className={`text-yellow-500/90 font-bold uppercase text-[12px] ${language === 'zh' ? 'tracking-[0.5em]' : 'tracking-[0.2em]'}`}>
                                    {t.achievements.athleteRank}
                                </span>
                            </div>
                            <h3 className="text-2xl font-serif text-white font-medium leading-tight relative z-10 tracking-wider">
                        ...
                        <div className="space-y-4">
                             <p className="text-[14px] text-gray-500 uppercase tracking-widest font-bold">{t.achievements.baseLocations}</p>
                                ...
                                {locations.map((loc, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className={`flex items-center gap-3 p-3 rounded-lg border ${isIntl ? 'bg-tennis-green/5 border-tennis-green/20' : 'bg-black/20 border-white/5'}`}>
                                            ...
                                            {isIntl && <span className="ml-auto text-[12px] uppercase font-bold text-tennis-green tracking-widest border border-tennis-green/20 px-2 py-0.5 rounded">{t.achievements.primary}</span>}
                                        </div>
                                    )
                                })}
                             </div>
                        ...
                        <div className="space-y-4 mt-auto">
                            <p className="text-[14px] text-gray-500 uppercase tracking-widest font-bold">{t.achievements.focusAreas}</p>
                        ...
                        {/* Coaching Formats (One-on-one / Group) */}
                        <div className="mt-6">
                            <p className="text-[14px] text-gray-500 uppercase tracking-widest font-bold">{t.achievements.coachingFormats}</p>
                            <div className="flex gap-2 mt-2">
                                {COACHING_INFO.formats[language].map((f, idx) => (
                                    <span key={idx} className="bg-white/5 border border-white/10 px-3 py-1.5 rounded-md text-xs font-medium text-gray-300 hover:bg-white/10 transition-colors">{f}</span>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="mt-6 pt-6 border-t border-white/10">
                            <div className="flex items-center gap-3">
                                <Mail className="w-5 h-5 text-tennis-green" />
                                <a 
                                    href="mailto:2311339135@qq.com"
                                    className="text-sm font-medium text-tennis-green hover:text-white transition-colors"
                                >
                                    2311339135@qq.com
                                </a>
                            </div>
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-center gap-4 min-w-[120px]">
                                    ...
                                    <span className="font-mono text-sm font-bold text-yellow-500/70">
                                        {item.year}
                                    </span>
                                </div>
