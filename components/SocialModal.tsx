    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-[#1e1e1e] border border-white/10 rounded-xl p-6 max-w-md w-full relative shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label className="block text-xs text-gray-500 uppercase font-bold mb-1">{t.social.formPlatform}</label>
                <select 
                ...
                >
                    <option value="youtube">{language === 'zh' ? 'YouTube' : 'YouTube'}</option>
                    <option value="instagram">{language === 'zh' ? 'Instagram' : 'Instagram'}</option>
                    <option value="bilibili">{language === 'zh' ? '哔哩哔哩' : 'Bilibili'}</option>
                    <option value="xiaohongshu">{language === 'zh' ? '小红书' : 'Xiaohongshu (Little Red Book)'}</option>
                    <option value="douyin">{language === 'zh' ? '抖音' : 'Douyin'}</option>
                    <option value="tiktok">{language === 'zh' ? 'TikTok' : 'TikTok'}</option>
                    <option value="weibo">{language === 'zh' ? '微博' : 'Weibo'}</option>
                    <option value="twitter">{language === 'zh' ? 'Twitter / X' : 'Twitter / X'}</option>
                    <option value="linkedin">{language === 'zh' ? 'LinkedIn' : 'LinkedIn'}</option>
                    <option value="other">{language === 'zh' ? '网站 / 其他' : 'Website / Blog / Other'}</option>
                    <option value="other">{language === 'zh' ? '播客' : 'Podcast'}</option>
                    <option value="other">{language === 'zh' ? '店铺' : 'Store / Shop'}</option>
                </select>
            </div>

            <div>
                <label className="block text-xs text-gray-500 uppercase font-bold mb-1">{t.social.formHandleLabel}</label>
                <input 
                    type="text" 
                    required
                    ...
                    className="w-full bg-black/30 border border-gray-700 rounded px-3 py-2 text-white text-sm"
                    placeholder={t.social.placeholderHandle}
                />
            </div>

            <div>
                  <label className="block text-xs text-gray-500 uppercase font-bold mb-1">{t.social.formUrlLabel}</label>
                <input 
                    type="text" 
                    required
                    value={formData.url}
                    ...
                    className="w-full bg-black/30 border border-gray-700 rounded px-3 py-2 text-white text-sm"
                    placeholder={t.social.placeholderUrl}
                />
            </div>

            <div>
                  <label className="block text-xs text-gray-500 uppercase font-bold mb-1">{t.social.formFollowersLabel}</label>
                <input 
                    type="text" 
                    value={formData.followers}
                    onChange={e => setFormData({...formData, followers: e.target.value})}
                    className="w-full bg-black/30 border border-gray-700 rounded px-3 py-2 text-white text-sm"
                    placeholder={t.social.placeholderFollowers}
                />
            </div>
